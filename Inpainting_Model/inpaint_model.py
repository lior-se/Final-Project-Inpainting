...
class InpaintCAModel(Model):
    ...
    def build_graph_with_losses(
        ...
        # Percentage for using the specific mask vs random masks
        specific_mask_percentage = 0.4  # Adjust this as needed
        ...
        batch_complete = batch_predicted * mask + batch_incomplete * (1. - mask)
        # local patches
        losses['ae_loss'] = FLAGS.l1_loss_alpha * tf.reduce_mean(tf.abs(batch_pos - x1)) #use of l1
        losses['ae_loss'] += FLAGS.l1_loss_alpha * tf.reduce_mean(tf.abs(batch_pos - x2))
        if summary:
            scalar_summary('losses/ae_loss', losses['ae_loss'])
            if FLAGS.guided:
    ...
    """def build_graph_with_losses(
        ...
        # Compute SSIM loss
        ssim_loss_x1 = 1 - tf.reduce_mean(tf.image.ssim(batch_pos, x1, max_val=2.0))
        ssim_loss_x2 = 1 - tf.reduce_mean(tf.image.ssim(batch_pos, x2, max_val=2.0))
        losses['ssim_loss'] = FLAGS.ssim_loss_alpha * (ssim_loss_x1 + ssim_loss_x2)

        if summary:
            scalar_summary('losses/ssim_loss', losses['ssim_loss'])
            if FLAGS.guided:
                viz_img = [
                    batch_pos,
                    batch_incomplete + edge,
        ...
        if summary:
            gradients_summary(losses['g_loss'], batch_predicted, name='g_loss')
            gradients_summary(losses['g_loss'], x2, name='g_loss_to_x2')
            gradients_summary(losses['ssim_loss'], x2, name='ssim_loss_to_x2')

        losses['g_loss'] = FLAGS.gan_loss_alpha * losses['g_loss']
        if FLAGS.ae_loss:
            losses['g_loss'] += losses['ssim_loss']
        g_vars = tf.compat.v1.get_collection(
            tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, 'inpaint_net')
        d_vars = tf.compat.v1.get_collection(
            tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, 'discriminator')
